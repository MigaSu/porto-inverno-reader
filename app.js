...
(function() {
  ...
  // =========================================================================
  // SWADE DICE ROLLER CONTROLLER
  // =========================================================================
  const DiceRoller = {
    storageKey: 'porto_dice_history_v2',
    state: {
    ...
    getFormulaString() {
      ...
      if (this.state.wildCard) {
        return `d${this.state.traitDie} + d6 (дикий)${modStr}`;
      }
      return `d${this.state.traitDie}${modStr}`;
    },

    updateFormulaPreview() {
      const hint = document.getElementById('diceFormulaHint');
      if (hint) hint.textContent = this.getFormulaString();
    },
    ...
    executeRoll(isBenny = false) {
      ...
      // Critical Failure: Snake eyes (initial rolls are both 1)
      ...
      if (isCritical) {
        verdictTitle = '💀 КРИТИЧЕСКИЙ ПРОВАЛ';
        verdictDesc = 'Оба кубика показали 1 при первом броске (Snake Eyes)! Автоматический критический провал. Переброс за фишку невозможен.';
        badgeType = 'critical';
      } else {
        verdictTitle = `ВЫПАЛО: ${finalTotal}`;
        const winnerName = winner === 'trait' ? `кубик характеристики (d${this.state.traitDie})` : `Дикий кубик (d6)`;
        const winnerScore = winner === 'trait' ? traitRoll.total : wildRoll.total;
        
        let descParts = [`Победил ${winnerName}: <strong>${winnerScore}</strong>.`];
        if (this.state.modifier !== 0) {
          descParts.push(`Модификатор: <strong>${this.state.modifier > 0 ? '+' : ''}${this.state.modifier}</strong>.`);
        }
        
        const aces = [];
        if (traitRoll.aced) aces.push(`кубик d${this.state.traitDie} взорвался (${traitRoll.rolls.join('!') + '!'})`);
        if (wildRoll && wildRoll.aced) aces.push(`дикий d6 взорвался (${wildRoll.rolls.join('!') + '!'})`);
        if (aces.length > 0) {
          descParts.push(`💥 ${aces.join(', ')}.`);
        }

        descParts.push(`Итоговое значение: <strong>${finalTotal}</strong>.`);
        verdictDesc = descParts.join(' ');
        badgeType = (traitRoll.aced || (wildRoll && wildRoll.aced)) ? 'raise-epic' : 'raise';
      }
      ...
      const bennyBtn = document.getElementById('btnExecuteBenny');
      if (bennyBtn) {
        bennyBtn.style.display = isCritical ? 'none' : 'flex';
      }
    },
    ...
    renderResult(roll) {
        ...
        <div class="foundry-card">
          <div class="foundry-card-header">
            <span class="foundry-card-brand">
              <span>🎲</span>
              <span>РЕЗУЛЬТАТ БРОСКА${roll.isBenny ? ' (ФИШКА / BENNY)' : ''}</span>
            </span>
          ...
          <div class="foundry-card-body">
            <div class="foundry-dice-grid">
              ...
              <div class="foundry-die-box ${roll.winner === 'trait' ? 'winner' : (roll.winner === 'wild' ? 'loser' : '')}">
                ...
                <span class="die-box-tag">Кубик характеристики (d${roll.traitDie})</span>
              ...
              ` : `
                <div class="foundry-die-box loser" style="justify-content:center; text-align:center;">
                  ...
                  <span style="color:var(--text-tertiary); font-size:0.85rem;">Отключен</span>
                </div>
              `}
            </div>
            ...
            <div class="foundry-calc-line">
              <span>Базовый результат: <strong>${winnerBase}</strong></span>
              ${mod !== 0 ? `<span>${mod > 0 ? '+' : '−'} Модификатор: <strong>${Math.abs(mod)}</strong></span>` : ''}
              <span>= Итоговое значение: <strong>${roll.finalTotal}</strong></span>
            </div>

            <!-- Outcome Message Banner -->
            <div class="foundry-outcome-banner ${roll.badgeType}">
              <div class="outcome-title">${roll.verdictTitle}</div>
              <div class="outcome-desc">${roll.verdictDesc}</div>
            </div>

            <!-- Actions Footer -->
            <div class="foundry-card-actions">
              <button type="button" class="btn-foundry-copy" id="btnCopyRollText" style="width: 100%; justify-content: center;">
                <span>📋</span>
      ...
      if (btnTxt) {
        btnTxt.addEventListener('click', () => {
          const plainDesc = roll.verdictDesc.replace(/<[^>]*>/g, '');
          const text = `[SWADE] ${roll.formula} ➔ ${roll.finalTotal} (${plainDesc})`;
      ...
      list.innerHTML = this.state.history.map(item => `
        <div class="history-item-row">
          ...
          <div class="history-right">
            <span class="history-total" style="font-size:1.15rem; font-weight:800; color:var(--gold-accent);">${item.finalTotal}</span>
          </div>
        </div>
      `).join('');
    },
